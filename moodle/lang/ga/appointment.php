<?php // $Id: appointment.php,v 1.1.2.3 2006/02/06 09:59:46 moodler Exp $
      // appointment.php - created with Moodle 1.0.5 dev (2002092300)


$string['allowresubmit'] = "Ceadaigh ath-aighniú";
$string['appointmentdetails'] = "Sonraí na coinne";
$string['appointmentname'] = "Ainm na coinne";
$string['appointmentlocation'] = "Láthair na coinne";
$string['description'] = "Cur síos";
$string['date'] = "Dáta na coinne";
$string['timestart'] = "Tús na coinne";
$string['timeend'] = "Deireadh na coinne";
$string['early'] = "\$a luath";
$string['failedupdatefeedback'] = "Níor éirigh leis an nuashonrú ar an aiseolas ar aighneacht do úsáideoir \$a";
$string['feedback'] = "Aiseolas";
$string['feedbackupdated'] = "Aiseolas ar na haighneachtaí nuashonraithe do \$a daoine";
$string['late'] = "\$a déanach";
$string['maximumgrade'] = "Uasghrád";
$string['maximumsize'] = "Uasmhéid";
$string['modulename'] = "Coinne";
$string['modulenameplural'] = "Coinní";
$string['newsubmissions'] = "Coinní aighnithe";
$string['notgradedyet'] = "níl sé grádaithe go fóill";
$string['notsubmittedyet'] = "níl sé aighnithe go fóill";
$string['overwritewarning'] = "Rabhadh: SCRIOSFAR d\'aighneacht reatha agus tógfaidh an aigneacht nua a h-áit má dheanann tú uaslódáil arís";
$string['saveallfeedback'] = "Sábháil m\'aiseolas ar fad";
$string['submissionfeedback'] = "Aiseolas ar an aighneacht";
$string['submissions'] = "Aighneachtaí";
$string['submitappointment'] = "Aighnigh do coinne ag baint úsáid as an bhfoirm seo";
$string['submitted'] = "Aighnithe";
$string['typeoffline'] = "Gníomhaíocht as líne";
$string['typeuploadsingle'] = "Uaslódáil comhad shingil";
$string['uploadbadname'] = "This filename contained strange characters and couldn't be uploaded";
$string['uploadedfiles'] = "comhaid uaslódáilte";
$string['uploaderror'] = "Tharla earráid nuair a bhí an comhad á shábháil ar an bhfreastalaí";
$string['uploadfailnoupdate'] = "File was uploaded OK but could not update your submission!";
$string['uploadfiletoobig'] = "Tá bron orm, ach tá an comhad sin ró-mhór (teorainn \$a bearta)";
$string['uploadnofilefound'] = "Ní raibh aon comhad aimsithe - an bhfuil tú cinnte gur roghnaigh tú ceann le huaslódáil?";
$string['uploadnotregistered'] = "Bhí '\$a' uaslódáilte i gceart ach níor chláraigh an aighneacht i gceart!";
$string['uploadsuccess'] = "D\'éirigh leis an uaslódáil do '\$a' ";
$string['viewfeedback'] = "Féach ar grádanna agus aiseolas ar na coinní";
$string['viewsubmissions'] = "Féach ar \$a coinní aighnithe";
$string['yoursubmission'] = "D\'aighneacht";

?>
