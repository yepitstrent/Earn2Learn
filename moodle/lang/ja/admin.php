<?PHP // $Id: admin.php,v 1.32.2.5 2006/02/06 09:59:55 moodler Exp $ 
...
$string['confightmleditor'] = '内蔵れたHTMLテキストエディタ使用を許可するかどうかを選択してください。「許可する」を選択した場合、互換性のあるブラウザが使われる場合のみ、このエディタが表示されます。ユーザはエディタを使用しない選択もできます。';
...
$string['configmaxbytes'] = 'ここではサイト全体でアップロードできるファイルの最大サイズを設定します。この設定はPHP設定のupload_max_filesizeおよびApache設定のLimitRequestBodyに制限されます。同様にmaxbytesは、コースレベルまたはモジュールレベルで選択するサイズ幅を制限します。';
...
$string['configsmtpuser'] = '上記でSMTPサーバを設定してなおかつ認証が必要な場合はユーザ名とパスワードを設定してください。';
...
$string['mysql416required'] = 'Moodle1.6では、将来全てのデータをUTF-8に変換するため、MySQL 4.1.16 が要求される最低限のバージョンです。';
...
$string['stickyblocksduplicatenotice'] = 'ここで特定のページに追加したブロックがすでに存在する場合、重複することになります。<br />固定されたブロックのみが編集不可で、重複したブロックは編集可能です。';
$string['stickyblocksmymoodle'] = 'マイ moodle';
$string['stickyblockspagetype'] = '設定するページタイプ';
$string['tabselectedtofront'] = 'タグ付きのテーブルで、現在選択されているタグの列を最前面に表示します。';
