...
function e2l_get_reward(){
    $dollar = $this->assignment->dollar; 
    if($dollar == null){
        $dollar = "0.00";
    }
    return $dollar;
...
function e2l_view_intro() {
	...
	$str = $this->assignment->description;
	echo $str;
    }
...
function insert_reward($amt, $uid){
//echo "in reward uid:".$uid." amt".$amt;
    ...
    $sql = "SELECT rewards FROM mdl_user WHERE id = ".$uid;
...
//echo var_dump($num);
        ...
        $sql = "UPDATE mdl_user SET rewards = ".$num." WHERE id = ".$uid;
    ...
    function update_main_listing($submission) {
        global $SESSION;
//echo "IN UPDATE".var_dump($this->assignment->dollar);
//echo var_dump($this);

        $this->insert_reward($this->assignment->dollar, $submission->userid);
        ...
        echo "\n-->\n</script>";
//	echo "BOTTOM".$SESSION->flextable['mod-assignment-submissions']->collapse['grade'];
        flush();
    }
	...
	//echo var_dump($USER);    
        ...
        print_user_picture($teacher->id, $this->course->id, $teacher->picture);
//	echo var_dump($this->assignment->dollar);
        echo '</td>';
        echo '<td class="content">';
...
//        echo '<input type="submit" name="submit" value="'.get_string('savechanges').'" onclick = "document.submitform.menuindex.value = document.submitform.grade.selectedIndex";  />';
        echo "<input type=\"submit\" name=\"submit\" value=\"".get_string('savechanges')."\" onclick=\"grade_task('".$this->assignment->dollar."', '".$user->id."')\"/>";
...
<script>
function grade_task(dollar, uid){
    document.submitform.menuindex.value = document.submitform.grade.selectedIndex;
}
</script>
