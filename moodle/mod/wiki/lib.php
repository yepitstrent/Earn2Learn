<?php  // $Id: lib.php,v 1.25.2.1 2005/07/05 08:17:29 skodak Exp $
...
function wiki_admin_checklinks($pagetocheck) {
  ...
  if($pagetocheck) {
     ...
     /// Remove old Notices
     $content = eregi_replace(' 도__~\['.get_string("offline","wiki").'\]__도 ','', $content);

     #-- replace dead links
     foreach ($badlinks as $href) {
        $content = preg_replace("\377^(.*)($href)\377m", '$1 도__~['.get_string("offline","wiki").']__도 $2', $content);
     }
