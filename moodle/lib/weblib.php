<?php // $Id: weblib.php,v 1.469.2.41 2006/04/20 22:30:53 martinlanghoff Exp $
 ...
 * @version  $Id: weblib.php,v 1.469.2.41 2006/04/20 22:30:53 martinlanghoff Exp $
...
function Earn2Learn(){
    ...
    $sql = "SELECT mu.id as id, mu.username, mc.fullname FROM mdl_user_students as mus, mdl_course as mc, mdl_user as mu WHERE mu.id = mus.userid AND mus.course = mc.id AND mu.id = ".$USER->id;
    $result = $conn->query($sql);

    if ($result->num_rows > 0) {
        // output data of each row
        while($row = $result->fetch_assoc()) {
            echo "id: " . $row["id"]. " - uname: " . $row["username"]." - cid: ".$row['fullname']."<br>";
        }
    } else {
        echo "0 results";
    }
...
function GetMyCourses(){
    ...
    if ($result->num_rows > 0) {
        ...
        while($row = $result->fetch_assoc()) {
            //echo "id: " . $row["id"]. " - uname: " . $row["username"]." - cid: ".$row['fullname']."<br>";
            $courses[] = array($row["id"], $row["username"], $row["fullname"]);
